...
export default function EventListItem({ event, displayCurrency, exchangeRate }: Props) {
  ...
  const price = event.tickets.priceInCents !== undefined ? `US$ ${event.price.toFixed(2)}` : 'Precio no disponible';

  console.log(event.tickets.priceInCents)


  const convertedPrice = (event.tickets.priceInCents * (exchangeRate || 1)).toFixed(2);
  console.log(displayCurrency, convertedPrice)
  return (
    <TouchableOpacity
